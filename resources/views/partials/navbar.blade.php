...
<div class="collapse navbar-collapse"
    ...
    @guest
        ...
        <div class="d-flex gap-3">
            ...
            <a class="btn btn-outline-primary {{ request()->routeIs('login') ? 'active' : '' }}"
    ...
    @else
        ...
        <ul class="navbar-nav ms-auto mb-2 mb-lg-0 me-4">
            ...
            @if (Auth::user()->hasRole('host'))
                ...
                <li class="nav-item">
                    <a class="nav-link {{ request()->is('usuario/anfitrion/mis-proyectos*') ? 'active' : '' }}"
                       href="{{ route('host.my-projects.index') }}">Mis Proyectos</a>
                </li>
                <li class="nav-item">
                    <a class="nav-link {{ request()->is('usuario/anfitrion/mi-perfil*') ? 'active' : '' }}"
                       href="{{ route('host.my-profile.profile') }}">Mi Perfil</a>
                </li>
            @endif
            @if (Auth::user()->hasRole('volunteer'))
                <li class="nav-item">
                    <a class="nav-link {{ request()->routeIs('volunteer.dashboard') ? 'active' : '' }}"
                       href="{{ route('volunteer.dashboard') }}">Dashboard</a>
                </li>
                <li class="nav-item">
                ...
                <li class="nav-item">
                    <a class="nav-link {{ request()->routeIs('volunteer.projects.applied') ? 'active' : '' }}"
                       href="{{ route('volunteer.projects.applied') }}">Mis Postulaciones</a>
                </li>
                <li class="nav-item">
                    <!-- mio perfil -->
                    <a class="nav-link {{ request()->is('usuario/voluntario/mi-perfil*') ? 'active' : '' }}"
                       href="{{ route('volunteer.my-profile.profile') }}">Mi Perfil</a>
                </li>
            @endif
        </ul>
        <div class="d-flex gap-3">
            <form method="POST"
                ...
                @csrf
                <button type="submit"
                        class="btn btn-outline-primary">Salir</button>
            </form>
        </div>
    @endguest
</div>
