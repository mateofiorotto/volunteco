<div id="volunteers-list">
    <div class="d-flex justify-content-end gap-2 p-2 ms-auto">
        <div class="text-muted small"><span class="d-inline-block rounded-circle bg-primary" style="width: 10px; height: 10px;"></span> Aceptado</div>
                                    ...
                                    <div class="d-flex justify-content-center gap-3">
                                        ...
                                        <div class="text-center">
                                            <form method="POST"
                                                ...
                                                @method('PUT')
                                                <button type="submit"
                                                        class="btn btn-sm btn-outline-secondary {{$volunteer->user->status !== 'activo' ? 'disabled' : ''}} {{ $project->enabled == false ? 'disabled' : '' }}">Rechazar</button>
                                            </form>
                                        </div>
                                        <div class="text-center">
                                            <form method="POST"
    ...
    <div class="card p-0 border-primary">
        <div class="card-header text-bg-primary">
            <div class="d-flex justify-content-between">
                <h2 class="h5 mb-0">Voluntarios <span class="fw-light">del projecto (<span class="small fw-light">Total: {{$inProject->count()}}</span>)</span></h2>
                <p class="mb-0"><span class="small fw-light">Activos: {{$accepted->count()}}</span> {!! $finished->count() ? '| <span class="small fw-light">Finalizados: '.$finished->count().'</span>' : '' !!}</p>
            </div>
        </div>
        <div class="card-body">
            ...
            <table class="table border-primary">
                <thead>
                    <tr>
                        ...
                        <th scope="col" class="text-center" width="440">Acciones</th>
                    </tr>
                </thead>
                <tbody>
                            ...
                            <td>
                                @if($volunteer->pivot->isCanceled() || $volunteer->pivot->isCompleted())
                                    ...
                                    <div class="d-flex gap-4 align-items-center">
                                        <p class="mb-0">Nivel: <span class="text-muted small">{{ $volunteer->evaluation->performance_label }} ({{ $volunteer->evaluation->average_score }})</span></p>
                                    ...
                                    <div class="col-6 text-center">
                                        ...
                                        @else
                                            ...
                                            <form method="POST"
                                                ...
                                                @method('PUT')
                                                <button type="submit"
                                                        class="btn btn-sm btn-danger {{$volunteer->user->status !== 'activo' ? 'disabled' : ''}} {{ $project->enabled == false ? 'disabled' : '' }}">Cancelar voluntariado</button>
                                            </form>
                                            @endif
                                        @endif
                                    </div>
                                    <div class="col-6 text-center">
                                        ...
                                        @else
                                            ...
                                            <form method="POST"
                                                ...
                                                @method('PUT')
                                                <button type="submit"
                                                        class="btn btn-sm btn-azul {{$volunteer->user->status !== 'activo' ? 'disabled' : ''}} {{ $project->enabled == false ? 'disabled' : '' }}">Completó el voluntariado</button>
                                            </form>
                                            @endif
                                        @endif
                                    </div>
