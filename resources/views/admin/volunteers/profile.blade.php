                                            ...
                                            <div class="row gap-5">
                                                <div class="col">
                                                    <ul class="list-unstyled">
                                                        ...
                                                        <li><a href="tel:{{ $volunteer->phone }}" target="_blank">{{ $volunteer->phone }}</a></li>
                                                        <li>{{ $volunteer->location->name ?? 'Sin ubicación' }} - {{$volunteer->location->province->name}}</li>
                                                    </ul>
                                                    <ul class="list-unstyled">
                                                        <li><span class="text-muted small">DNI: </span>{{ number_format($volunteer->dni, 0, ',', '.') }}</li>
                                                        <li class="text-nowrap"><span class="text-muted small">Fecha de nacimiento: </span>{{ $volunteer->birthdate->format('d/m/Y') }} ({{ $volunteer->birthdate->age }} años)</li>
                                                    </ul>
                                                </div>
                                                <div class="col">
                                                    <ul class="list-unstyled mb-0">
                                                        <li><span class="text-muted small">Profesión: </span><span class="text-capitalize">{{ $volunteer->profession ?? 'Sin profesión' }}</span></li>
                                    ...
                                    <div class="card mb-3">
                                        ...
                                        <div class="card-body">
                                            ...
                                            <ul class="list-unstyled mb-0">
                                                ...
                                                <li>
                                                    <a href="{{ $volunteer->linkedin }}" target="_blank" class="text-nowrap"><i class="bi bi-linkedin fs-5 me-2 text-azul"></i> {{ $volunteer->linkedin }}</a>
                                                </li>
                                                @endif

                                                @if ($volunteer->instagram)
                                                <li>
                                                    <a href="{{ $volunteer->instagram }}" target="_blank" class="text-nowrap"><i class="bi bi-instagram fs-5 me-2 text-azul"></i> {{ $volunteer->instagram }}</a>
                                                </li>
                                                @endif
                                                @if ($volunteer->facebook)
                                                <li>
                                                    <a href="{{ $volunteer->facebook }}" target="_blank" class="text-nowrap"><i class="bi bi-facebook fs-5 me-2 text-azul"></i> {{ $volunteer->facebook }}</a>
                                                </li>
                                                @endif
                                            </ul>
                                            @else
                ...
                <div class="col-12 col-md-4">
                    <div class="card mb-3">
                        ...
                        <div class="card-body">
                            ...
                            @if ($volunteer->user->status !== 'activo')
                                <p class="alert {{$volunteer->user->status == 'pendiente' ? 'alert-warning' : 'alert-danger'}}">Este voluntario está {{$volunteer->user->status}}</p>
                                ...
                                <div class="border-top pt-3 d-flex justify-content-between">
                                    <form method="POST"
                                        ...
                                        @csrf
                                        @method('PUT')
                                        <button class="btn btn-outline-danger mb-3" type="submit">Reactivar</button>
                                    </form>
                                </div>
                                @if($volunteer->user->status == 'inactivo')
                                    <div class="pt-3 border-top">
                                        <form method="POST" class="d-flex flex-column" action="{{ route('admin.delete-volunteer-profile', $volunteer->user_id) }}">
                                            @csrf
                                            @method('DELETE')
                                            <div class="mb-3">
                                ...
                                @endif
                            @else
                                <form method="POST" action="{{ route('admin.disable-volunteer-profile', $volunteer->user_id) }}">
                                    @csrf
                                    @method('PUT')
