    ...
    <section>
        ...
        <div>
            @if ($host->enabled == false)
                <form method="POST"
                      action="/admin/verificar-perfil-anfitrion/{{ $host->id }}/aceptar">
                    @csrf
                    @method('PUT')
                    <button type="submit">Aceptar</button>
                </form>
                <form method="POST"
                      action="/admin/verificar-perfil-anfitrion/{{ $host->id }}/rechazar">
                    @csrf
                    @method('DELETE')
            ...
            @else
                <form method="POST"
                      action="/admin/verificar-perfil-anfitrion/{{ $host->id }}/desactivar">
                    @csrf
                    @method('PUT')
